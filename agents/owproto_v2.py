    ...
    for tick in range(1, ticks):
        ...
        for pid, is_moving, val1, val2, r_p in planet_params:
            if is_moving:
                ...
                p_x = 50 + val2 * math.cos(angle_t)
                p_y = 50 + val2 * math.sin(angle_t)
            else:
                p_x, p_y = val1, val2
...
def find_angle_to_moving_planet(p, t, ships, vel, planets):
...
def find_fast_reinforcement_ships(p_np, p, needed_by_tick, min_ships, max_ships, moving_planets, vel, planets):
    ...
    while low <= high:
        ...
        if p.id not in moving_planets:
            ...
            fleet_speed = 1.0 + (MAX_SPEED - 1.0) * (math.log(max(1, mid)) / math.log(1000)) ** 1.5                
            dist = math.sqrt((p.x - p_np.x)**2 + (p.y - p_np.y)**2)
            arrive_tick = math.floor(dist / fleet_speed)
            
            if path_collision(p_np, p, fleet_speed, angle_np, planets, vel, ticks=arrive_tick + 1):
                angle_np = None
                arrive_tick = None
        else:
            angle_np, arrive_tick = find_angle_to_moving_planet(p_np, p, mid, vel, planets)
...
def agent(obs):
    ...
    sorted_plans = sorted(reinforcement_plans.items(), key=lambda x: x[0].production, reverse=True)
    for p, plan in sorted_plans:
        ...
        for row in nearest_planets:
            ...
            if p.id not in moving_planets:
                ...
                if path_collision(p_np, p, fleet_speed, angle_np, lobs.get("planets", []), obs.angular_velocity, ticks=arrive_tick + 1):
                    angle_np = None
                    arrive_tick = None

                if arrive_tick is None or arrive_tick > needed_by_tick:
                    boosted_ships = find_fast_reinforcement_ships(p_np, p, needed_by_tick, sent_reinforcements, p_np_available_ships, moving_planets, obs.angular_velocity, lobs.get("planets", []))
                    if boosted_ships is not None:
                        sent_reinforcements = boosted_ships
                        fleet_speed = 1.0 + (MAX_SPEED - 1.0) * (math.log(sent_reinforcements) / math.log(1000)) ** 1.5
                        arrive_tick = math.floor(dist / fleet_speed)
                        angle_np = math.atan2(p.y - p_np.y, p.x - p_np.x)
                    else:
                ...
                angle_np, arrive_tick = find_angle_to_moving_planet(p_np, p, sent_reinforcements, obs.angular_velocity, lobs.get("planets", []))
                if angle_np is None or arrive_tick is None or arrive_tick > needed_by_tick:
                    boosted_ships = find_fast_reinforcement_ships(p_np, p, needed_by_tick, sent_reinforcements, p_np_available_ships, moving_planets, obs.angular_velocity, lobs.get("planets", []))
                    if boosted_ships is not None:
                        sent_reinforcements = boosted_ships
                        angle_np, arrive_tick = find_angle_to_moving_planet(p_np, p, sent_reinforcements, obs.angular_velocity, lobs.get("planets", []))
        ...
        for m, t, s in candidate_targets[:3]:
            ...
            if m_available_ships >= base_ships: # single attack
                if t.id in moving_planets: # single moving planet
                    ...
                    for _ in range(3):
                        angle, arrive_tick = find_angle_to_moving_planet(m, t, total_ships, obs.angular_velocity, lobs.get("planets", []))
                ...
                if angle is not None and arrive_tick is not None:
                    ...
                    collides_path = path_collision(m, t, fleet_speed, angle, lobs.get("planets", []), obs.angular_velocity, ticks=arrive_tick + 1)
                    if collides_path:
                        continue
                    ...
                    if t.id not in moving_planets: # coop static planet
                        if t.owner == -1: # coop static unowned
                            ...
                            for a_p in attacking_planets:
                                ...
                                collides_path = path_collision(p, t, fleet_speed=fleet_speed, angle=angle, planets=lobs.get("planets", []), vel=obs.angular_velocity, ticks=arrive_tick + 1)
                                if collides_path:
                                    break
                        ...
                        if t.owner == -1: # coop moving unowned
                            ...
                            for a_p in attacking_planets:
                                ...
                                angle, arrive_tick = find_angle_to_moving_planet(p, t, p_ships, obs.angular_velocity, lobs.get("planets", []))
