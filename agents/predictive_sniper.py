...
def simulate_state_forward(planets, fleets, angular_velocity, comets, comet_planet_ids, max_steps, max_speed=6.0):
    """A highly lightweight 50-step rollout simulator for the physical state of the game."""
    sim_planets = [[p.id, p.owner, p.x, p.y, p.radius, p.ships, p.production] for p in planets]
    sim_fleets = [[f.id, f.owner, f.x, f.y, f.angle, f.from_planet_id, f.ships] for f in fleets]
...
def apply_action_to_state(action, planets_state, fleets_state, player, next_fleet_id):
    """Mutates simulation state to dispatch a candidate fleet action."""
    new_fleets = list(fleets_state)
    for move in action:
        ...
        if planet and planet[1] == player and planet[5] >= ships and ships > 0:
            ...
            new_fleets.append([next_fleet_id, player, start_x, start_y, angle, from_id, ships])
            next_fleet_id += 1
    return new_fleets, next_fleet_id

def generate_candidate_actions(player, planets, fleets, obs, timelines, available_surplus, is_opponent=False):
    ...
    for target, score in scored_targets:
        if coord_count >= max_coord:
            break
        for T in range(15, 45, 10):  # Ticks: 15, 25, 35
...
def agent(obs):
    ...
    # Identify dominant opponent player to model minimax decisions
    opponents = [i for i in range(4) if i != player]
    opponent_ships = {o: 0 for o in opponents}
    for p in planets:
        if p.owner in opponent_ships:
            opponent_ships[p.owner] += p.ships
    opponent = max(opponent_ships, key=lambda o: opponent_ships[o])

    # Generate Candidate Action Sets
    ally_candidates = generate_candidate_actions(player, planets, fleets, obs, timelines, available_surplus, is_opponent=False)
    ...
    enemy_candidates = generate_candidate_actions(opponent, planets, fleets, obs, timelines, opp_surplus, is_opponent=True)

    # --- Phase 3: 2-Ply Minimax Search Rollouts ---
    best_move = []
    best_score = -float('inf')
    next_fleet_id = get(obs, "next_fleet_id", len(fleets) + 1)
    angular_velocity = get(obs, "angular_velocity", 0.03)
    comets = get(obs, "comets", [])
    comet_planet_ids = get(obs, "comet_planet_ids", [])

    for ally_move in ally_candidates:
        min_score = float('inf')
        for enemy_move in enemy_candidates:
            # Clone and setup simulated states
            sim_planets = [[p.id, p.owner, p.x, p.y, p.radius, p.ships, p.production] for p in planets]
            sim_fleets = [[f.id, f.owner, f.x, f.y, f.angle, f.from_planet_id, f.ships] for f in fleets]
            
            # Dispatch candidate moves
            # Convert planets back to named Planet tuples to pass into apply_action safely
            named_planets = [Planet(*p) for p in sim_planets]
            named_fleets = [Fleet(*f) for f in sim_fleets]
            
            # Ally launch
            named_fleets, temp_fid = apply_action_to_state(ally_move, sim_planets, named_fleets, player, next_fleet_id)
            # Enemy launch
            named_fleets, temp_fid = apply_action_to_state(enemy_move, sim_planets, named_fleets, opponent, temp_fid)

            # Re-convert sim_fleets
            sim_fleets = [Fleet(*f) for f in named_fleets]
            sim_planets_named = [Planet(*p) for p in sim_planets]

            # Simulate state forward 40 ticks
            rollout_planets, rollout_fleets = simulate_state_forward(
                sim_planets_named, sim_fleets, angular_velocity, comets, comet_planet_ids, max_steps=40
            )
